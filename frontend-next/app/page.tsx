...
function statusTone(status: string) {
  ...
  if (["running", "retrying", "index_queued", "indexing", "partially_covered", "review_required"].includes(status)) return "warn";
  return "neutral";
}

function Tag({ value }: { value: string }) {
  const labels: Record<string, string> = { index_queued: "Queued", indexing: "Indexing", indexed: "Indexed", index_failed: "Failed" };
...
function Knowledge({ token, documents, baselines, reviews, refresh, selectReview }: { token: string; documents: DocumentVersion[]; baselines: RequirementBaseline[]; reviews: ReviewPackage[]; refresh: () => Promise<void>; selectReview: (id: string) => void }) {
  const [notice, setNotice] = useState<string | null>(null); const [error, setError] = useState<string | null>(null); const [busy, setBusy] = useState(false); const [optimisticIndexIds, setOptimisticIndexIds] = useState<Set<string>>(() => new Set()); const [selectedBaseline, setSelectedBaseline] = useState<RequirementBaseline | null>(null); const [requirements, setRequirements] = useState<Requirement[]>([]); const [requirementsLoading, setRequirementsLoading] = useState(false);
  ...
  const hasActiveIndex = optimisticIndexIds.size > 0 || documents.some((document) => ["index_queued", "indexing"].includes(document.ingestion_status));
  useEffect(() => { if (!hasActiveIndex) return; const timer = window.setInterval(() => void refresh(), 2000); return () => window.clearInterval(timer); }, [hasActiveIndex, refresh]);
      ...
      <div className="stack">
        ...
        <section className="panel">
          ...
          <div className="doc-list">{activeDocuments.length ? activeDocuments.map((document) => {
            ...
            return <div className="doc-row" key={document.id}><div><b>{document.title}</b><span>{documentTypeLabel(document.document_type)} · {systemLabel(document.system)} · v{document.version} · {document.chunk_count} chunks</span>{document.ingestion_error && <small title={document.ingestion_error}>{document.ingestion_error}</small>}</div><Tag value={document.status} /><Tag value={ingestionStatus} />{indexActionAvailable && <button className="secondary" disabled={busy || indexingActive} onClick={() => void indexDocument(document.id)}>{ingestionStatus === "index_failed" ? "Retry index" : "Create index"}</button>}<button className="secondary" disabled={busy || indexingActive} onClick={() => void reparseDocument(document)}>Reparse source</button><button className="secondary" disabled={busy || indexingActive} onClick={() => void archiveDocument(document)}>Archive</button></div>;
